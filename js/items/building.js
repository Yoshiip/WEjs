class Building {
    ...
    draw(ctx, viewPoint, enable3d = true) {
        if(!enable3d) {
            this.base.draw(ctx, { fill: "white", stroke: "gray" });
            return;
        }
        ...
        for (const poly of roofPolys) {
            var roofColor = [["#D44", "#C44"], ["#4D4", "#4C4"], ["#44D", "#44C"]][Math.floor(magnitude(this.base.points[0])) % 3];
            poly.draw(ctx, { fill: roofColor[0], stroke: roofColor[1], lineWidth: 8, join: "round" });
        }
    }
}